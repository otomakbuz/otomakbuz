...
export async function generateInvoicePdf(
  ...
  pdf.text(company.name || "Firma", margin, y + 6);
  y += 10;

  pdf.setFontSize(8);
  pdf.setFont("helvetica", "normal");
  if (company.company_tax_id) pdf.text(`VKN: ${company.company_tax_id}`, margin, y);
  if (company.company_tax_office) pdf.text(`V.D.: ${company.company_tax_office}`, margin + 40, y);
  y += 4;
  if (company.company_address) { pdf.text(company.company_address, margin, y); y += 4; }
  if (company.company_phone) pdf.text(`Tel: ${company.company_phone}`, margin, y);
  if (company.company_email) pdf.text(`E-posta: ${company.company_email}`, margin + 60, y);
  y += 8;
